  ...
  theme: {
    extend: {
      ...
      colors: {
        ...
        // ── 다온은행(타행 시연) ──
        //
        // **AXful 팔레트와 섞지 않는다.** 다온은 다른 은행이라 디자인이 달라야 한다.
        // 값이 kb-admin 과 같지만(둘 다 #1B3A6B) 뜻이 다르다 — 같은 토큰을 빌려 쓰면
        // AXful 색을 바꿀 때 다온이 따라 바뀐다. 실제로 그렇게 만들어 놓았다가
        // 되돌렸다.
        "daon-navy":       "#1B3A6B",  // 다온 브랜드 네이비
        "daon-navy-mid":   "#384D84",  // 그라디언트 중간
        "daon-navy-light": "#5A73A8",  // 그라디언트 끝
        // 표면색. 값은 지금 AXful 베이지와 같지만 이름을 나눠 둔다 — 다온을
        // 시각적으로 더 구분하고 싶으면 여기만 바꾸면 된다.
        "daon-surface":       "#F5F0E8",
        "daon-surface-light": "#FAFAF7",
