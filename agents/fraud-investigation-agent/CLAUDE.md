...
1. **결정적 사실은 코드.** 조사 중 사망·후견이 확인되면 LLM 판단과 무관하게 즉시
   ...
   LLM 이 관여하는 곳은 셋뿐이다 — **가설 판단**, **도구 선택**, 그리고 고객 안내
   문구의 **말투 다듬기**(`guidance.refine`). 셋째는 판단이 아니다: 무엇이 위험한지와
   무엇을 해야 하는지는 탐지기가 규칙으로 정하고, 모델 결과는 `_safe_merge` 가 걸러
   근거·선택지를 원래대로 되돌린다. 항목 수가 달라지면 통째로 버린다. 모델이 죽거나
   꺼져 있어도 규칙 결과가 그대로 답이라 **안내가 비는 일은 없다** — 차단만 되고
   이유가 없는 화면은 고치려던 문제보다 나쁘기 때문이다.
...
│   └── guidance.py      # 소비자 안내 다듬기(연령대별)                 [구현]
├── scripts/             # CLI 러너 등
├── tests/
└── data/cases/          # 목 사건 데이터
```
